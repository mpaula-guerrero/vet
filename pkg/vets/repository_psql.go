...
func (s *psqldb) Create(m *Vet) error {
	const sqlInsert = `INSERT INTO public.vets (nombre,created_at, update_at) VALUES (:nombre, Now(), Now()) `
...
func (s *psqldb) Update(m *Vet) error {
	const sqlUpdate = `UPDATE public.vets SET nombre = :nombre , update_at = Now() WHERE id = :id `
...
func (s *psqldb) GetByID(id int) (*Vet, error) {
	const sqlGetByID = `SELECT id, nombre, created_at, update_at FROM public.vets WHERE id = $1 `
	mdl := Vet{}
...
func (s *psqldb) GetAll() ([]*Vet, error) {
	const sqlGetAll = `SELECT id, nombre, created_at, update_at FROM public.vets `
	ms := []*Vet{}
